...
test('synthetic demo: eight questions, conservative results, activities, and actual PDF download', async ({ page }, testInfo) => {
  ...
  const output = `.impeccable/review/didactic-2026-09-08/${testInfo.project.name}`;
  const errors: string[] = [];
  const writes: string[] = [];
  ...
  const modelExplanation = page.getByRole('region', { name: 'Sobre el análisis de texto' });
  await expect(modelExplanation).toHaveCount(1);
  await expect(modelExplanation).toContainText('Es un límite del modelo');
  await expect(modelExplanation.getByRole('link', { name: 'Completar mi cuestionario' })).toBeVisible();
  await expect(page.getByText('evidencia insuficiente — sin cifra', { exact: true })).toHaveCount(0);
  await expect(page.locator('[data-testid^="bf-value-"]')).toHaveCount(0);
  await capture('model-explanation');
  ...
  await expect(bigFive).toContainText('Es un límite del modelo');
  await expect(bigFive.locator('.pdf-dimension')).toHaveCount(0);
