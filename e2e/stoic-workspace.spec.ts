...
for (const width of [390, 768, 1292, 1440]) {
  test(`guided workspace stays usable at ${width}px`, async ({ page }, testInfo) => {
    ...
    await expect(page.getByRole('button', { name: 'Leer mi resultado' })).toBeVisible();
    await capture('dashboard');
    await page.getByRole('button', { name: 'Leer mi resultado' }).click();
    await expect(page.getByRole('tabpanel', { name: 'Tu lectura' })).toBeFocused();
    ...
    await expect(page.getByRole('tab', { name: 'Datos del modelo', exact: true })).toBeFocused();
