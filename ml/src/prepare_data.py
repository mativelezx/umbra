"""Prepare data: une corpus Essays + corpus latinoamericano, genera splits.
...
def load_latinoamericano() -> pd.DataFrame:
    ...
    df = pd.read_csv(path)
    df["origin"] = "latinoamericano"
    df["language"] = "es-AR"
    log.info("latinoamericano: %d casos cargados", len(df))
    return df
...
def load_essays() -> Optional[pd.DataFrame]:
    ...
    interfaz que latinoamericano.
    """
    path = DATA / "essays" / "essays.csv"
    if not path.exists():
        log.warning("Essays no disponible en %s — se entrena solo con latinoamericano", path)
        return None
    df = pd.read_csv(path)
...
def main():
    ...
    if essays is not None:
        ...
        df = rio.copy()
        log.warning("Entrenando solo con corpus latinoamericano (n=%d). Las métricas "
