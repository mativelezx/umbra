...
- rioplatense_only — solo casos del corpus rioplatense propio
...
def main():
    ...
    if es_mask.sum() >= 2:
        out["blocks"]["rioplatense_only"] = evaluate_block(
            models, test[es_mask].reset_index(drop=True),
            (X_test[es_mask] if model_type == "distilbert_ridge" else X_test[es_mask]),
            "rioplatense_only",
        )
    else:
        out["blocks"]["rioplatense_only"] = {"n_samples": int(es_mask.sum()),
                                             "note": "muestras de rioplatense en test insuficientes (n<2)"}
    ...
    out["dataset_status"] = {
        ...
        "warning": (
            ...
            "en el corpus rioplatense propio (n=20 etiquetado IPIP). El poder "
