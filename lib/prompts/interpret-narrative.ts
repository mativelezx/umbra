 ...
 * delega a Claude la lectura interpretativa: funciones cognitivas Jung,
 * arquetipo Pearson y razonamiento citando evidencia textual del
 * usuario. Reemplaza la inferencia conjunta del viejo
 * `analyze-profile.ts` (deprecated post pivot ML).
 *
 * Encuadre crítico (ADR-002 v2 + ADR-007 amendado): este prompt explicita
 * que las funciones cognitivas Jung y el arquetipo NO son mediciones
 * psicométricas — son lecturas interpretativas heurísticas derivadas
 * de los Big Five inferidos por el módulo ML y de los textos del usuario.
 */
...
> automáticamente como dimensión propia (ADR-002 v2 + ADR-007 amendado).
...
Breve (2-3 párrafos) en español rioplatense con voseo. Cita evidencia
...
- Español rioplatense (voseo) en el reasoning.
