 ...
 *
 * **MVP STATUS (2026-04-14)**: Current corpus has 33 cases (ipip-18..20,
 * jung-01..20, adv-01..10). Cases ipip-01..17 are pending authoring and
 * are required to reach the preregistered 50-case target before H1/H2
 * final evaluation. The runner accepts any corpus size; short-term runs
 * report the actual n used and flag it as MVP in the output JSON. The
 * OSF preregistration text should be finalized only once the corpus
 * matches the preregistered size.
 *
 * Initial draft generated by codex (OpenAI gpt-5.4, reasoning high,
 * 2026-04-14) following a prompt that specified dimension targets,
 * ethics constraints, and voseo rioplatense voice. Tail truncation
 * dropped the first 17 IPIP cases from the output; those are marked as
 * TODO in the corpus count (but the existing 33 cases are committed
 * verbatim as draft quality is high).
 *
