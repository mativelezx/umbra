...
export function CrisisCard({ resources, severity, extraMessage }: CrisisCardProps) {
  return (
    <div
      role="alert"
      className="relative rounded-lg border-2 border-accent-amber/40 bg-umbra-abyss/90 p-6 md:p-8 "
    >
      <div className="mb-6 flex items-start gap-4">
        ...
        <div>
          <h2 className="text-balance font-heading font-semibold text-3xl not-italic text-text-1 md:text-4xl">
          ...
          {extraMessage && (
            <p className="mt-3 text-pretty font-body text-xs not-italic text-text-3">{extraMessage}</p>
          )}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        {resources.map((r) => (
          ...
          >
            <div>
              <div className="font-heading text-sm text-text-1 group-hover:text-text-1">
                {r.label}
              </div>
              {r.region && (
                <div className="mt-0.5 font-body text-sm text-text-3">
                  {r.region} {r.hours && `· ${r.hours}`}
                </div>
              )}
            </div>
            <div className="flex items-center gap-2 font-body text-sm text-accent-amber">
      ...
      <div className="mt-8 flex flex-wrap items-center justify-between gap-4 border-t border-accent-amber/20 pt-6">
        <p className="font-body text-sm text-text-4">severidad registrada: {severity}</p>
        <Link
          href="/"
