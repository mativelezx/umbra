...
it('attributes its suggestions to an experimental reading instead of stating traits as facts', () => {
  render(<ContextualGreeting profile={{ firstName: 'Ana', archetype: 'sage', bigFive: DEMO_BIG_FIVE, jungFunctions: DEMO_JUNG_FUNCTIONS, perDimensionStatus: { ...RIDGE_V1_STATUS, openness: 'ok' } }} />);
...
it('does not infer a Big Five trait when confidence evidence is missing', () => {
  render(<ContextualGreeting profile={{ firstName: 'Ana', archetype: 'sage', bigFive: DEMO_BIG_FIVE, jungFunctions: DEMO_JUNG_FUNCTIONS }} />);
  expect(screen.queryByText(/estimación experimental de Big Five/)).not.toBeInTheDocument();
  expect(screen.queryByText(/ejemplo para contrastar con tu experiencia/)).not.toBeInTheDocument();
  expect(screen.getByText(/interpretación de IA para explorar/)).toBeVisible();
});
